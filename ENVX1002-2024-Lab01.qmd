...
[download data file](./data/Lead_content.csv)
...
Specialised software is needed to write out equations when writing documents and reports. One option is Microsoft Equation Editor which comes as part of Microsoft Word. You may use this function in this or other units to write out equations.
...
To begin with we are going to open R ("Engine") from the program files menu on your computer.
...
- R can accept data from many different sources; for example directly from scientific instruments or even scraping the internet for data.  In this topic we are only considering small data sets so we will enter the data manually via the keyboard.
...
![New Quarto](images/QuartoDoc.png)
...
![Render Quarto](images/Quartorender.png)
